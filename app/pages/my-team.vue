...
const teamCounts = () => {
  const constructorCards = Object.values(userObj.value?.currentTeam || {}).filter((c) => c?.cardData.type === CardType.CONSTRUCTOR);
  const driverCards = Object.values(userObj.value?.currentTeam || {}).filter((c) => c?.cardData.type === CardType.DRIVER);
