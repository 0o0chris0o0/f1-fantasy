<template>
  <div>
    ...
    <div class="flex flex-col gap-1 justify-center items-center uppercase font-semibold pt-2" v-if="showModifierInfo">
      <div class="grid grid-cols-12 gap-1">
        <div class="col-span-3">
          <Icon name="game-icons:round-star" :class="{
        ...
        <div class="col-span-4">
          <p class="text-xs">{{ `+${getModifierValues().levelModifier * 100}%` }}</p>
        </div>

        <div class="col-span-3">
          <Icon name="game-icons:house" :class="{
        ...
        <div class="col-span-4">
          <p class="text-xs">{{ `+${getModifierValues().homeRaceModifier * 100}%` }}</p>
        </div>
...
import { iCardRarity, type iCardInUsersCards } from '@f1pick6/shared';
...
const cardScoreModifier = computed(() => {
  ...
  return calcCurrentModifierScore(props.data, props.currentRound, userObj.value?.currentTeam).totalScoreModifier;
})

const getModifierValues = () => {
  if (!userObj.value?.currentTeam) return {
    rarityModifier: 0,
    levelModifier: 0,
    homeRaceModifier: 0,
    teamMatchModifier: 0
  ...
  return calcCurrentModifierScore(props.data, props.currentRound, userObj.value?.currentTeam);
}
