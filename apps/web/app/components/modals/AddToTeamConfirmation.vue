    ...
    <template #content>
      <div v-if="cardData">
            ...
            <div class="space-y-1">
              <p>
                ...
                <span class="font-normal"
                  >P{{ cardData.stats.averageQualifyingPosition }}</span
                >
              </p>
              <p>
                Average Race Pos:
                <span class="font-normal"
                  >P{{ cardData.stats.averageRacePosition }}</span
                >
              </p>
              <p>
                DNF's:
        ...
        <div class="grid grid-cols-1 text-center gap-4">
          <div v-if="addToTeam && props.userData">
            <Button @click="addToTeam(props.userData)">Add to Team</Button>
          </div>
          <div>
            <Button @click="props.close" version="grey" size="small"
              >Cancel</Button
            >
          </div>
        </div>
...
const props = defineProps<{
  ...
  close?: () => void;
  addToTeam?: (card: iCardInUsersCards) => void;
}>();
