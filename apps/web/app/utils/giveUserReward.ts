  ...
  switch (rewardObject.rewardType) {
    ...
    case RewardType.CARDS:
      ...
      const randomDriverCard =
        driverCards[Math.floor(Math.random() * driverCards.length)];
      const randomConstructorCard =
        constructorCards[Math.floor(Math.random() * constructorCards.length)];

      if (!userObj.value?.cards || !randomDriverCard || !randomConstructorCard)
        return;
      ...
      [randomDriverCard, randomConstructorCard].forEach((c) => {
        ...
        lootCards.push({
          loot: newCardData,
          isNew:
            !!!userObj.value?.cardsHistory[`${c.cardId}_${rewardObject.key}`],
        });
      });
    ...
    case RewardType.PACK:
      ...
      // this pack is a free reward, don't charge the user for it
