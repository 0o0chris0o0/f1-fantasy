...
export async function updatePlayerScores(
  ...
  playersSnap.docs.forEach((player) => {
    ...
    const playerScores = generatePlayerScores(
      playerTeam,
      fantasyScores,
    ...
    const playersLeaderboardObj: Omit<
      iLeaderboardScore,
    ...
    > = {
      ...
      modifierScore:
        playerScores.totalModifiedScore - playerScores.baseFantasyScore,
    };
    ...
    const playersCurrentCardHistory = player.get("cardsHistory") as Record<
      string,
      iUserCardHistory
    >;
    ...
    playersTeam.forEach((card: iCardInUsersCards) => {
      ...
      const indexOfCard = playersCurrentCards.findIndex(
        (c: iCardInUsersCards) =>
      ...
      if (playersCurrentCardHistory[card.cardData.cardId]) {
        ...
        // only increase the level to a max of 4
        playersCurrentCardHistory[card.cardData.cardId].level +=
          playersCurrentCardHistory[card.cardData.cardId].level === 4 ? 0 : 1;
      } else {
