...
export async function updateAllCards(
  ...
  const driverCards: iDriverCard[] = [];
  const constructorCards: iConstructorCard[] = [];
  ...
  driverCards.forEach((card) => {
    const cardScores = fantasyScores[card.cardId] as iDriverFantasyScore;
    ...
    if (cardScores) {
      ...
      if (cardScores.constructor !== card.teamId) {
        logger.info(
          `${card.cardName} has a new team, moving to ${cardScores.constructorName}`,
        );
  ...
  if (Object.keys(clonedDriverScores).length > 0) {
    logger.info(
      "There are new drivers that we dont have cards for, creating new cards for them...",
    );
    logger.log(clonedDriverScores);
    const scheduleRef = await firestore.collection("schedule").get();
    const scheduleData = scheduleRef.docs.map((doc) => doc.data() as iRace);
    ...
    for (const cardId in clonedDriverScores) {
      ...
      const driverResponse = await axios(
        `https://api.jolpi.ca/ergast/f1/drivers/${cardId}/`,
      );
      const driverData = driverResponse.data.MRData.DriverTable.Drivers[0];
      let driverLocaleCode,
        driverCountry = "";
      let homeRaces: iRace[] = [];

      if (driverData) {
        nationalities.registerLocale(enLocale);
        driverLocaleCode = nationalities.getAlpha2Code(
      ...
      if (driverLocaleCode && driverCountry) {
        logger.info(
          `Nationality info found for ${cardScores.driverName}: ${driverLocaleCode}: ${driverCountry}`,
        );
        logger.info("Getting schedule data to find home races...");
        homeRaces = scheduleData.filter(
          (race) => race.locationCountry === driverCountry?.toLowerCase(),
        );
      } else {
        logger.warn(
          `Couldn't find nationality info for ${cardScores.driverName}, setting to empty string`,
        );
      }
      ...
      logger.info(
        `Creating new card for ${cardScores.driverName} with ID ${cardId}`,
      );
  ...
  constructorCards.forEach((card) => {
    const cardScores = fantasyScores[card.cardId] as iConstructorFantasyScore;
    ...
    if (cardScores) {
      ...
      // update drivers
      const constructorDrivers = driverCards.filter(
        (driverCard) => driverCard.teamId === card.teamId,
      );
  ...
  playersRef.docs.forEach((doc) => {
    ...
    const newPlayersCard = mergeUpdatedCards(playersCards, updatedCards);
...
export function mergeUpdatedCards(
  playersCards: iCardInUsersCards[],
  updatedCards: Record<string, iDriverCard | iConstructorCard>,
) {
  return playersCards.map((card) => {
    const newCardData = updatedCards[card.cardData.cardId];

    return newCardData ? { ...card, cardData: newCardData } : card;
  });
}
